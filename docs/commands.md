...
4. **Runner.** `.github/workflows/run-thread.yml` builds a matrix from `links` and runs one job per URL in parallel (`max-parallel: 16`, `fail-fast: false`). Each shard posts progress, success, and failure callbacks to `/api/callback` with the original `commandType` and `actionType: "thread-single"` or `"thread-multi"`.
...
| Modal opens, user submits, but **nothing more happens** (`/threaddl`, `/threaddl-spoiler`) | A forged ModalSubmit with an unknown `customId` prefix is silently dropped by the allowlist check. Under normal use this only happens when the bot lacks **Send Messages in Threads**: the thread is created, the `🧵 Created thread` follow-up posts, but every per-URL `sendMessage` rejects, the placeholders are dropped silently, and the flow returns without dispatching. See [deployment.md](./deployment.md#bot-permissions-in-discord). |
