...
| `tests/scripts/` | Shell script と AWK tool tests（`deno test` with subprocess execution）：`progress.awk` parsing、`retry_curl.sh` retry logic、`post_process.sh` file handling、`conv_progress.sh` text conversion。 |
| `.github/scripts/` | Runner workflows で使用される Shell scripts と AWK tools：`progress.awk`（yt-dlp progress を parse）、`retry_curl.sh`（robust HTTP POST with backoff）、`post_process.sh`（file staging と multipart assembly）、`conv_progress.sh`（text encoding と Discord-safe formatting）。 |
| `.github/actions/check-and-convert-files/action.yml` | Composite GitHub Action。`post_process.sh` を実行して、ファイルを validate、convert、check。`run.yml` と `run-thread.yml` から呼び出し。 |
