...
| `src/bot/bot.ts` | Creates the discordeno bot with `intents: Intents.Guilds` (slash commands are dispatched to the gateway; the bot does not listen to guild messages or message content). Wires `interactionCreate` to dispatch by `interaction.type`. `ModalSubmit` interactions go straight to `threadModalSubmit`. `ApplicationCommand` interactions are routed by command name to `interactionCreate` (`/dl`, `/dl-spoiler`) or `threadInteractionCreate` (`/threaddl`, `/threaddl-spoiler`). No top-level `await` (importing `bot.ts` is side-effect-free, which is what makes it testable). |
...
| `src/bot/commands.ts` | Slash command definitions for `dl`, `dl-spoiler`, `threaddl`, `threaddl-spoiler`. The two thread commands are guild-only via `dmPermission: false` (threads can only be created in guild text/announcement/forum channels, not in DMs). All four commands declare their options and descriptions for Discord's API registration. |
...
| `tests/scripts/` | Shell script and AWK tool tests via `deno test` with subprocess execution: `progress.awk` ffmpeg output parsing, `retry_curl.sh` retry logic, `post_process.sh` format conversion, `conv_progress.sh` callback dispatch, `run.sh` workflow orchestration, and `check_and_convert_files.sh` size/conversion flow. |
| `.github/scripts/` | Shell scripts and AWK tools used by runner workflows: `progress.awk` (parses ffmpeg encoding progress output), `retry_curl.sh` (robust HTTP POST with backoff), `post_process.sh` (H.264/yuv420p format validation and conversion using libx264), `conv_progress.sh` (monitors ffmpeg progress log file and dispatches HTTP callbacks to the bot), `run.sh` (shared workflow orchestration), and `check_and_convert_files.sh` (10 MB size check plus two-pass conversion). |
| `.github/actions/check-and-convert-files/action.yml` | Composite GitHub Action that forwards its inputs to `check_and_convert_files.sh`, which performs two-pass HEVC (libx265) + Opus re-encoding to fit files within the 10 MB Discord limit. Called by `run.yml` and `run-thread.yml`. |
...
| `.github/workflows/test.yml` | CI: `bash -n` runner-script validation → `deno lint` → `deno task test` → `deno task test:coverage`, with the coverage report appended to the GitHub Step Summary. Tests all modules including `tests/scripts/`. |
